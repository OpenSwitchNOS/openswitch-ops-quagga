...
BGP multiple-paths carry risks of routing oscillations in some scenario, for example, route-reflection. Even BGP aggregates the paths that form the ECMP when advertising on. If IGP topology and BGP topology are at odds with each other with BGP speakers having only a subset of the path information (e.g. route-reflection and/or ECMP), then oscillation can occur.
