...
int bgp_bfd_neigh_add(struct peer *peer)
{
  ...
  if (!CHECK_FLAG (peer->flags, PEER_FLAG_BFD)) {
    VLOG_INFO("BFD is not enabled for peer=%s\n", peer->host);
    printf("BFD is not enabled for peer=%s\n", peer->host);
    return 1;
  }

  if (!peer->su_local) {
    VLOG_INFO("Waiting to create BFD session .. su_local is Invalid\n");
    printf("Waiting to create BFD session .. su_local is Invalid\n");
    return 1;
  }

  if (!peer->su_remote) {
    VLOG_INFO("Waiting to create BFD session .. su_remote is Invalid\n");
    printf("Waiting to create BFD session .. su_remote is Invalid\n");
    return 1;
  }

  bgp_create_bfd_session_in_ovsdb(peer);
#else
...
int bgp_bfd_neigh_del(struct peer *peer)
{
  ...
  if (!peer->su_remote) {
    VLOG_INFO("BFD session not created for %s, nothing to do here\n", peer->host);
    return 1;
  }
  ...
  VLOG_INFO("Deleting BFD session for  AS=%d host=%s peer=%s\n",
		  peer->local_as, peer->host, remote_str);

  bgp_delete_bfd_session_in_ovsdb(remote_str);
  return 0;
}
...
daemon_neighbor_bfd_fallover_enable_cmd_execute (struct bgp *bgp, char *peer_str, bool fall_over)
{
    ...
    VLOG_DBG("neighbor %s fall-over bfd %s \n",
...
daemon_neighbor_bfd_state_cmd_execute (struct bgp *bgp, char *peer_str, int bfd_state)
{
    ...
    if (bfd_state == PEER_BFD_STATUS_UP) {
	ret = peer_flag_unset(peer, PEER_FLAG_SHUTDOWN);
    } else {
        ret = peer_flag_set(peer, PEER_FLAG_SHUTDOWN);
    }

    // Update BFD status in BGP neighbor table
    bgp_update_bfd_status_in_ovsdb(peer, bfd_state);
