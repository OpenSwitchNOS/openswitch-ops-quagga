...
{
    /* fall-over */
    if (COL_CHANGED(ovs_nbr, ovsrec_bgp_neighbor_col_fall_over_bfd, idl_seqno)) {
	bool is_set = (ovs_nbr->fall_over_bfd) ? true : false;

        VLOG_DBG("Fallover BFD %s for %s", (is_set) ? "Enable" : "Disable", name);
        daemon_neighbor_fall_over_bfd_cmd_execute(bgp_instance, name, is_set);
    }
}
...
bgp_apply_bfd_neighbor_changes (struct ovsdb_idl *idl)
{
	...
	OVSREC_BFD_SESSION_FOR_EACH(ovs_bfd_session, idl) {
		...
		if (strcmp(ovs_bfd_session->owner, OVSREC_BFD_SESSION_OWNER_BGP) != 0) {
...
bgp_create_bfd_session_in_ovsdb(char *remote, char *local, as_t asn)
{
    ...
    if (ovs_bfd_session) {
      ...
      if (ovs_bfd_session) {
          ...
          ovsrec_bfd_session_set_bfd_src_ip(ovs_bfd_session, local);
          ovsrec_bfd_session_set_owner(ovs_bfd_session, OVSREC_BFD_SESSION_OWNER_BGP);
...
bgp_update_bfd_status_in_ovsdb (struct peer *peer, int  bfd_state)
{
    ...
    if (ovs_nbr != NULL) {
	    ovsrec_bgp_neighbor_set_fall_over_bfd_status(ovs_nbr,
