...
#define MAX_LEN (diag_buffer_len - len)
/*
    ...
    const char *argv[] OVS_UNUSED, void *aux OVS_UNUSED)
{
    ...
    snprintf(buf, sizeof(buf), "Set diag buffer to %d bytes", diag_buffer_len);
    unixctl_command_reply(conn, buf);
...
static int
bgp_dump_peers(char *buf, int offset, struct peer *p)
{
    struct bgp *bgp;
    char buf1[BUFSIZ];
    char timebuf[BGP_UPTIME_LEN];
...
bgpd_dump(char *buf, int buf_size)
{
    ...
    int afi, safi, len = 0;
    bgp = bgp_get_default ();
    char *err = "Diag-dump data exceeds";
    if (bgp) {
        ...
        for (ALL_LIST_ELEMENTS (bgp->peer, node, nnode, peer))
        {
            ...
            len += bgp_dump_peers(buf, len, peer);
        }
        if(len >= diag_buffer_len) {
            VLOG_ERR("diag-dump: No more space on buffer to dump."
                     "  Dumped %d bytes of data.\n", len);
            snprintf(&buf[diag_buffer_len - strlen(err) - 25], MAX_LEN,
                            "...\n%s %d bytes.", err, diag_buffer_len);
        }
        VLOG_DBG("\nDumped %d bytes out of %d\n", len, diag_buffer_len);
