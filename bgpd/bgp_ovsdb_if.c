...
bgp_apply_bgp_neighbor_changes (struct ovsdb_idl *idl)
{
    ...
    if (deleted) {
        ...
        delete_bgp_neighbors_and_peer_groups(db_bgpn_p, idl);
    }

    /* nothing else changed ? */
    if (!modified && !inserted) return;

    VLOG_DBG("processing bgp neighbor modifications\n");
	...
	/*
	** If this is a new row and remote-as exists,
	** it MUST be the first time creation of a
	** bgp neighbor.  If remote-as is not there, then
	** it MUST be the first time creation of a peer group.
	** if remote_as is specified, it must
	** be a neighbor else peer group.  Also assert
	** the extra conditions too.
	*/
	remote_as_done = false;
	if (NEW_ROW(db_bgpn_p, idl_seqno)) {

	    /* object is a peer/neighbor */
	    if (db_bgpn_p->n_remote_as) {
		assert(!object_is_peer_group(db_bgpn_p));
		daemon_neighbor_remote_as_cmd_execute(bgp_instance,
		    db_bgpn_p->name, db_bgpn_p->remote_as,
		    AFI_IP, SAFI_UNICAST);
		remote_as_done = true;

	    /* object is a peer group */
	    } else {
		assert(object_is_peer_group(db_bgpn_p));
	    ...
	    if (!ROW_CHANGED(db_bgpn_p, idl_seqno))
		continue;
	}
	...
	/* remote-as */
	if (!remote_as_done &&
	    COL_CHANGED(db_bgpn_p, ovsrec_bgp_neighbor_col_remote_as, idl_seqno)) {
	...
	if (COL_CHANGED(db_bgpn_p, ovsrec_bgp_neighbor_col_inbound_soft_reconfiguration,
	    idl_seqno)) {
		bool enable =
	...
	if (db_bgpn_p->timers) {
	    ...
	    keepalive = smap_get(&db_bgpn_p->timers, "Keepalive");
	    holdtime = smap_get(&db_bgpn_p->timers, "Holdtime");
