...
update_active_bgp_router_id (const struct ovsrec_bgp_router *bgp_router_row,
    ...
    VLOG_INFO("BGP received active router id update : %s", active_router_id);

    /* If no router ID configured then update new router id */
    if (bgp_router_row != NULL && active_router_id != NULL){

        VLOG_INFO("Current value of BGP router_id: %s", bgp_router_row->router_id);
        ...
        for (ALL_LIST_ELEMENTS (bgpmaster->bgp, node, nnode, bgp)){
            if (bgp->router_id_static.s_addr == 0){
                ...
                update_db = true;
                VLOG_INFO("Setting router id for BGP ASN: %d", bgp->as);
                break;
            }
        }
...
/*        if (update_db){
            ...
            ovsrec_bgp_router_set_router_id (bgp_router_row, active_router_id);
