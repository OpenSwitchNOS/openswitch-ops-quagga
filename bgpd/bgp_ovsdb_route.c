...
policy_community_filter_read_ovsdb_apply_changes(struct ovsdb_idl *idl)
{
    ...
    OVSREC_BGP_COMMUNITY_FILTER_FOR_EACH(ovs_cfilter, idl) {
        if (ovs_cfilter->n_permit !=0) {
            direct = COMMUNITY_PERMIT;
            if (strcmp(ovs_cfilter->type,"extcommunity-list") == 0) {
                for (i = 0; i < ovs_cfilter->n_permit; i++) {
                    ...
                    if (!ret) {
                        VLOG_DBG("Extcommunity filter permit configuration changes set");
                     }
                }
            } else if (strcmp(ovs_cfilter->type,"community-list") == 0) {
                for (i = 0; i < ovs_cfilter->n_permit; i++) {
                    ...
                    if (!ret) {
                        VLOG_DBG("Community filter permit configuration changes set");
                     }
                }
            }
        }
        if (ovs_cfilter->n_deny !=0) {
            ...
            if (strcmp(ovs_cfilter->type,"extcommunity-list") == 0) {
                for (i = 0; i < ovs_cfilter->n_deny; i++) {
                    ret = extcommunity_list_set (bgp_clist,
                                 ...
                                 EXTCOMMUNITY_LIST_EXPANDED);
                    if (!ret) {
                        VLOG_DBG("Extcommunity filter deny configuration changes set");
                     }
                ...
                for (i = 0; i < ovs_cfilter->n_deny; i++) {
                    ret = community_list_set (bgp_clist,
                                 ...
                                 COMMUNITY_LIST_EXPANDED);
                    if (!ret) {
                        VLOG_DBG("Community filter deny configuration changes set");
                     }
                }
            }
        }
