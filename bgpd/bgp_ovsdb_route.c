...
static bool lookup_aspath_filter_match_from_ovsdb(const struct ovsrec_bgp_aspath_filter *flist, const char *match)
{
    int itr;
    if (match) {
...
lookup_aspath_filter_from_ovsdb(const char *name, const char *match)
{
    ...
    OVSREC_BGP_ASPATH_FILTER_FOR_EACH(ovs_flist, idl) {
        if (!strcmp (name, ovs_flist->name) && (lookup_aspath_filter_match_from_ovsdb(ovs_flist, match) == true)) {
            return ovs_flist;
        }
    }
