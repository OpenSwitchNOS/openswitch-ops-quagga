...
uint32_t get_lookup_key(char *prefix, char *table_name);
                                 ...
                                 safi_t safi)
{
    ...
    HMAP_FOR_EACH_IN_BUCKET(g_var, node, lookup_hash, &global_hmap) {
        if(!strcmp(g_var->prefix, pr) && (g_var->table_type == BGP_ROUTE)){
            const struct ovsrec_bgp_route *ri_row = ovsrec_bgp_route_get_for_uuid(idl, &g_var->uuid);
            return ri_row;
        }
    }
    return NULL;
...
bgp_info_found(struct bgp *bgp, struct bgp_ovsdb_txn *txn)
{
    ...
    if (rn == NULL) {
        VLOG_INFO("BGP info found is NULL\n");
        return false;
    }
...
bgp_txn_complete_processing(void)
{
    ...
    HMAP_FOR_EACH (txn, hmap_node, &bgp_ovsdb_txn_hmap) {
        ...
        if ((status == TXN_SUCCESS) || (status == TXN_UNCHANGED) ||
            ...
            if(status == TXN_SUCCESS)
            {
                if (txn->request == TXN_BGP_ADD || txn->request == TXN_BGP_DEL || txn->request == TXN_BGP_UPD_ATTR){
                    table_type = BGP_ROUTE;
                    lookup_hash = get_lookup_key(prefix_str, BGP_ROUTE_TABLE);
                }
                else if (txn->request == TXN_BGP_UPD_ANNOUNCE || txn->request == TXN_BGP_UPD_WITHDRAW){
                    table_type = ROUTE;
                ...
                HMAP_FOR_EACH_IN_BUCKET (g_var, node, lookup_hash, &global_hmap) {
                    ...
                    if (!strcmp(g_var->prefix, prefix_str) && (table_type == g_var->table_type)){
                        ...
                        if (g_var->op_type == DELETE) {
                            ...
                            g_var->state = DB_SYNC;
                            g_var->needs_review = 0;
                            const struct uuid *db_uuid = ovsdb_idl_txn_get_insert_uuid(txn->txn, &(g_var->uuid));
...
bgp_review(struct bgp_ovsdb_txn *txn, enum txn_op_type op, bgp_table_type_t table_type)
{
    ...
    if (!bgp) {
        VLOG_INFO("BGP node is NULL for incoming prefix %s\n", pr_bgp);
        return false;
    }
