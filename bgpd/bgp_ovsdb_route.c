                             ...
                             safi_t safi)
{
    ...
    if (nexthop->s_addr == 0) {
        ...
        /* If ECMP is disable, only advertise 1 route to rib */
        if(!get_ecmp_status()) {
            if(bgp_info_mpath_count (info)> 0) {
                nexthop_num = 1;
...
int
bgp_ovsdb_readvertise_route(const struct ovsrec_bgp_router *bgp_first, int asn)
{
    struct bgp_node *rn;
    struct bgp_info *ri;
    struct bgp * bgp;
         ...
         rn = bgp_route_next (rn))
    {
        for (ri = rn->info;ri;ri = ri->next)
        {
            if(bgp_info_mpath_count(ri))
            {
                prefix2str(&rn->p, pr, sizeof(pr));
                VLOG_DBG("%s del route %s, has mpaths = %d > 1\n",__FUNCTION__,
