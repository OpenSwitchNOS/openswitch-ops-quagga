...
class bgpTest (HalonTest):
    ...
    def unconfigure_network_bgp (self):
        ...
        cfg_array.append("no router bgp %s" % BGP2_ROUTER_ID)

        SwitchVtyshUtils.vtysh_cfg_cmd(switch, cfg_array)

    def verify_neighbor_password(self):
        switch = self.net.switches[1]
        found = SwitchVtyshUtils.verify_bgp_route(switch, BGP1_NETWORK,
                                                          BGP1_ROUTER_ID)
    ...
    def change_password(self):
        ...
        cfg_array = []
        cfg_array.append("router bgp %s" % BGP1_ASN)
        cfg_array.append("neighbor %s password %s" % (BGP1_NEIGHBOR, BGP1_WRONG_PASSWORD))
        SwitchVtyshUtils.vtysh_cfg_cmd(switch, cfg_array)
    ...
    def verify_wrong_neighbor_password(self):
        ...
        info(found)
        assert found == False, "TCP connection should not be established(%s -> %s) on %s" % \
                      (BGP1_NETWORK, BGP1_ROUTER_ID, switch.name)
    ...
    def verify_no_neighbor_password(self):
        ...
        found = SwitchVtyshUtils.verify_bgp_route(switch, BGP1_NETWORK,
                                                          BGP1_ROUTER_ID)
        info("Verifying no neighbor password \n")
        info(found)
        assert found, "TCP connection could not be established(%s -> %s) on %s" % \
                      (BGP1_NETWORK, BGP1_ROUTER_ID, switch.name)
...
class Test_bgp:
    ...
    def test_bgp_full (self):
        ...
        self.test_var.change_password()
        sleep(10)
        self.test_var.verify_wrong_neighbor_password()
        self.test_var.change_to_no_neighbor_password()
        sleep(30)
