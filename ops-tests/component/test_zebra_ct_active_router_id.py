...
def test_zebra_ct_active_router_id(topology, step):
    ...
    loopback_ip = "9.0.0.3"
    bgp_router_id3 = "9.0.0.3"
    sw1("configure terminal")
    sw1("interface loopback 3")
    sw1("no shutdown")
    sw1("ip address {}/{}".format(loopback_ip, bgp_pl))
    sw1("exit")
    output = sw1("ovsdb-client dump VRF", shell='bash')
    assert bgp_router_id3 in output

    step("4-Verify deleting loopback interface changes the
            active router id")
    sw1("configure terminal")
    sw1("interface loopback 3")
    sw1("shutdown")
    sw1("no ip address {}/{}".format(loopback_ip, bgp_pl))
    sw1("exit")
    output = sw1("ovsdb-client dump VRF", shell='bash')
    assert bgp_router_id1 in output or bgp_router_id2 in output

    step("5-Verify deleting one of the interface changes the
            active router id")
    sw1("configure terminal")
    sw1("interface 1")
    sw1("shutdown")
