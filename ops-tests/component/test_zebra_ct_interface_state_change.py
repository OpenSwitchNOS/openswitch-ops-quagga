...
from helpers_bgp import verify_show
...
def interface_shut_trigger_static_routes(sw1, sw2, step):
    ...
    verify_show(sw1, aux_route, 'static', route_ipv4_static_route4,
                'ip route')
    aux_route = rib_ipv4_static_route4["Route"]
    verify_show(sw1, aux_route, 'static', rib_ipv4_static_route4, 'rib')
    ...
    verify_show(sw1, aux_route, 'static', route_ipv6_static_route4,
                'ipv6 route')
    aux_route = rib_ipv6_static_route4["Route"]
    verify_show(sw1, aux_route, 'static', rib_ipv6_static_route4, 'rib')
...
def interface_unconfiguring_addresses_trigger_static_routes(sw1, sw2, step):
    ...
    for i in range(size):
        ...
        sw1("no ipv6 address {}/{}".format(sw1_ifs_ipv6s[i], sw1_ipv6_mask))
        sw1("exit")
        output = sw1("do show running-config")
...
def interface_changing_addresses_trigger_static_routes(sw1, sw2, step):
    step("Entering interface for link 1 SW1, changing anip/ip6 address")
    sw1_interfaces = [sw1.ports["if01"], sw1.ports["if04"]]
...
def interface_changing_back_addresses_trigger_static_routes(sw1, sw2, step):
    step("Entering interface for link 1 SW1, changing back anip/ip6 address")
    sw1_interfaces = [sw1.ports["if01"], sw1.ports["if04"]]
