...
def test_ipv6_static_route_config(topology, step):
    ...
    sw2p1 = format(sw1.ports["if01"])
    sw2p2 = format(sw1.ports["if02"])
    ...
    sw2('interface %s' % sw2p1)
    sw2('ipv6 address 2000::2/120')
    sw2('no shutdown')
    sw2('exit')
    sw2('interface %s' % sw2p2)
    ...
    sw1('ipv6 route 2002::/120 %s 2' % sw1p2)
    ret = sw1('do show running-config')

    assert 'ipv6 route 2002::/120 %s 2' % sw1p2 in ret, \
           'IPv6 route configuration failed with nexthop[ interface'

    step('### Verify deletion of ipv6 route with nexthop interface ###')
    sw1('no ipv6 route 2002::/120 %s 2' % sw1p2)
    ret = sw1('do show running-config')

    assert 'ipv6 route 2002::/120 %s 2' % sw1p2 not in ret, \
           'Deletion of ipv6 routes failed with nexthop interface'

    step('### Verify setting of multiple nexthops for a given prefix ###')
    sw1('ipv6 route 2002::/120 %s' % sw1p1)
    sw1('ipv6 route 2002::/120 %s' % sw1p2)
    ret = sw1('do show running-config')

    assert 'ipv6 route 2002::/120 %s' % sw1p1 in ret and \
        'ipv6 route 2002::/120 %s' % sw1p2 in ret, \
