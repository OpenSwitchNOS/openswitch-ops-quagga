...
def wait_for_interface_up(sw1):
    ...
    for i in range(MAX_WAIT_INT_TIME):
        int_status_sw1 = sw1.libs.vtysh.show_interface('1')
        admin_state = int_status_sw1['admin_state']
        int_state = int_status_sw1['interface_state']
...
def ospf_wait_for_route_install(sw1, step):
    """
    ...
    for i in range(wait_time):
        rts = sw1("/sbin/ip netns exec swns route -n", shell="bash")
        if OSPF_ROUTE in rts:
            kernel_result = True
            break

        sleep(1)

    if ovsdb_result is True\
            and kernel_result is True:
        return True

    return False


def ospf_wait_for_route_withdrawal(sw1, step):
    """
    This function takes a switch and a step object as inputs and waits
...
def test_zebra_ospf_route(topology, step):
    ...
    vrf_uuid = get_vrf_uuid(sw1, VRF_NAME, step)
    port_uuid = get_port_uuid(sw1, INT1, step)
    assert wait_for_interface_up(sw1),\
    ...
    sw1(ospf_route_command, shell="bash")
    ret = ospf_wait_for_route_install(sw1, step)
    assert ret is True,\
        'Test OSPF route installation by zebra - Failed'

    step("### Test OSPF route deletion on interface shutdown ###")
    with sw1.libs.vtysh.ConfigInterface('if01') as ctx:
        ctx.shutdown()
    ret = ospf_wait_for_route_withdrawal(sw1, step)
    assert ret is True,\
        'Test OSPF route Deletion by zebra - Failed'

    step("### Test OSPF route re-installation on interface up ###")
    with sw1.libs.vtysh.ConfigInterface('if01') as ctx:
        ctx.no_shutdown()
    ret = ospf_wait_for_route_install(sw1, step)
    assert ret is True,\
    ...
    step("### Test OSPF route deletion by zebra ###")
    ospf_route_command = "ovsdb-client transact \'[ \"OpenSwitch\",\
        {\
            \"op\" : \"delete\",\
            \"table\" : \"Route\",\
