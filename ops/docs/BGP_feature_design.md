...
OpenSwitch is designed to achieve aggressive modularization, hight availabilty, portability, extensibility, and reuse of open source projects. Quagga BGP project is integrated as one of the modules. BGP alone can provide complete dynamic routing for OpenSwitch. It can also work together with other protocols such as OSPF, ISIS etc. This document mainly focus on the role that BGP plays in the OpenSwitch Architecture and its interaction with other modules. For BGP internal design, refer to BGP_DESIGN.md. For the details of any other module that partcicipates in refer to corresponding module DESIGN.md.
...
(Future project ?)
    - None stop forwarding or graceful restart (GR), GR not fully supported by Quagga currently
    - None stop routing (NSR) using multipath TCP ?
