...
def configuration(topology, request):
    ...
    sw1('show interface {p11}'.format(**locals()))
    sw1('show interface {p12}'.format(**locals()))
    sw2('show interface {p21}'.format(**locals()))
    sw2('show interface {p22}'.format(**locals()))
    sw3('show interface {p31}'.format(**locals()))
    sw3('show interface {p32}'.format(**locals()))
    sw4('show interface {p41}'.format(**locals()))
    sw5('show interface {p52}'.format(**locals()))
...
def test_ospf_abr(topology, configuration, step):
    ...
    retval = wait_for_adjacency(sw4, SW3_ROUTER_ID)
    if retval:
        step('Adjacency formed between SW4 and SW3(router-id = 3.3.3.3)')
    else:
        assert False, "Failed to form adjacency between"
    ...
    if retval:
        ...
        assert False, 'Connected route from sw4 to sw1 failed '
        'to be removed'
...
def test_ospf_routes_abr(topology, configuration, step):
    ...
    # wait to reboot the switch
    with sw1.libs.vtysh.Configure() as ctx:
        ctx.no_router_ospf()

    # Waiting time for connected routes of sw1 to be updated
    time.sleep(40)
    step(
        '###### Step 4 - Verifying the connected routes to sw1 '
        'using show rib command ######'
        )
    retval = verify_route(sw3, '10.10.10.0', '24')
    if retval:
        assert False, 'Failed to remove connected route from sw3 to sw1'
    else:
        step(
            '###### Connected route from sw3 to sw1 is removed '
            'after disabling OSPF in switch1 ######'
            )
    ...
    if retval:
        ...
        step(
            '###### Connected route from sw4 to sw1 is removed '
            'after disabling OSPF in switch1 ######'
            )
...
def test_ospf_summarized_routes(topology, configuration, step):
    ...
    if retval:
        ...
        assert False, "Failed to form adjacency between"
        "SW4 and SW3(router-id = %s)" % SW3_ROUTER_ID

    # TODO:Summarization is not yet supported
    '''
    step('###### Step3 - Verifying the connected routes in sw1 ######')
    ...
    step('###### Step 4 - Verify if the switches are ABR ######')
    retval = verify_router_type(sw2)
    if retval == 2:
        step('###### SW2 is ABR ######')
    else:
    ...
    '''
    step(
        '###### Step5 - Verifying the connected routes to sw1'
        'using show rib command ######'
        )
