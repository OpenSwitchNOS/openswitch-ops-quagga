...
from .ospf_configs import wait_for_best_route
...
def test_ospf_best_route(topology, configuration, step):
    ...
    step('######step 5 - Verifying the sw2 advertised routes in sw1 ######')
    ...
    step('######step 7 - Verifying statically configured '
         'local routes in SW1 ######')
    ...
    step('######step 8 - Verifying the statically configured'
         ' best route in SW1 ######')
    retval = wait_for_best_route(sw1, '192.168.1.0', '80')
    if retval is True:
        step('Static route 192.168.1.0/24 in SW1 with distance metrics *80* '
             'is selected as best route')
    else:
        sw1('show ip route'.format(**locals()), shell='vtysh')
        assert False, "Static route 192.168.1.0/24 in SW1 with "
