...
ospf_ovsdb_tables_init()
{
   /* Add VRF columns */
...
#if 0
    /* Add OSPF_Interface_Config columns */
    ...
    ovsdb_idl_omit_alert(idl, &ovsrec_ospf_interface_config_col_ospf_intervals);
    ovsdb_idl_add_column(idl, &ovsrec_ospf_interface_config_col_if_out_cost);
    ovsdb_idl_omit_alert(idl, &ovsrec_ospf_interface_config_col_if_out_cost);
    ovsdb_idl_add_column(idl, &ovsrec_ospf_interface_config_col_mtu_ignore);
    ovsdb_idl_omit_alert(idl, &ovsrec_ospf_interface_config_col_mtu_ignore);
    ovsdb_idl_add_column(idl, &ovsrec_ospf_interface_config_col_priority);
    ovsdb_idl_omit_alert(idl, &ovsrec_ospf_interface_config_col_priority);
    ovsdb_idl_add_column(idl, &ovsrec_ospf_interface_config_col_passive_interface);
    ovsdb_idl_omit_alert(idl, &ovsrec_ospf_interface_config_col_passive_interface);
#endif
    ...
    ovsdb_idl_add_table(idl, &ovsrec_table_ospf_nbma_neighbor_config);
    ovsdb_idl_add_column(idl, &ovsrec_ospf_nbma_neighbor_config_col_interface_name);
    ovsdb_idl_add_column(idl, &ovsrec_ospf_nbma_neighbor_config_col_other_config);
    ovsdb_idl_add_column(idl, &ovsrec_ospf_nbma_neighbor_config_col_status);
    ovsdb_idl_add_column(idl, &ovsrec_ospf_nbma_neighbor_config_col_nbr_router_id);
    ...
    ovsdb_idl_omit_alert(idl, &ovsrec_ospf_lsa_col_ls_age);
...
find_ospf_nbr_by_if_addr (const struct ovsrec_ospf_interface* ovs_intf, struct in_addr src)
{
    struct ovsrec_ospf_neighbor* nbr_row = NULL;
    int i = 0;

    for (i = 0 ; i < ovs_intf->n_neighbors ; i++)
    {
        nbr_row = ovs_intf->neighbors[i];
...
#if 0
     /* This condition checks if router_id is static or dynamic and
      * if dynamic is the ip same
      */
     if (!IPV4_ADDR_SAME(&addr,&(ospf_cfg->router_id)))
     {
        txn = ovsdb_idl_txn_create(idl);

        if(!txn)
        {
            VLOG_DBG ("Enable to create trnasaction");
            ovsdb_idl_txn_abort(txn);
            return -1;
        }

         smap_init(&routerid_smap);
         if (!isRouter_id_present)
         {
            //smap_add(&routerid_smap,"router_id_val",inet_ntoa(ospf_cfg->router_id));
            smap_clone(&routerid_smap,&(ospf_mod_row->router_id));
            smap_replace(&routerid_smap,"router_id_val",inet_ntoa(ospf_cfg->router_id));
            ovsrec_ospf_router_set_router_id(ospf_mod_row,&routerid_smap);

            status = ovsdb_idl_txn_commit_block(txn);
            if (TXN_SUCCESS != status &&
             TXN_UNCHANGED != status)
                 VLOG_DBG ("Commit unsuccessful");

            smap_destroy(&routerid_smap);
    ...
    const struct ovsrec_ospf_router *ospf_mod_row)
{
    ...
    for(i = 0 ; i < ospf_mod_row->n_stub_router_adv; i++)
    {
        if(strcmp(ospf_mod_row->key_stub_router_adv[i],
       OVSREC_OSPF_ROUTER_STUB_ROUTER_ADV_ADMIN)) {
                    if (ospf_mod_row->value_stub_router_adv[i] &&
                            !CHECK_FLAG(ospf_cfg->stub_router_admin_set,OSPF_STUB_ROUTER_ADMINISTRATIVE_SET)) {
                    SET_FLAG(ospf_cfg->stub_router_admin_set,OSPF_STUB_ROUTER_ADMINISTRATIVE_SET);
                                stub_admin_set = 1;
                    }
            else if (!ospf_mod_row->value_stub_router_adv[i] &&
                            !CHECK_FLAG(ospf_cfg->stub_router_admin_set,~OSPF_STUB_ROUTER_ADMINISTRATIVE_UNSET)){
                                UNSET_FLAG(ospf_cfg->stub_router_admin_set,OSPF_STUB_ROUTER_ADMINISTRATIVE_SET);
                                stub_admin_set = 0;
                     }
                }
        else if (strcmp(ospf_mod_row->key_stub_router_adv[i],
            OVSREC_OSPF_ROUTER_STUB_ROUTER_ADV_STARTUP)) {
                ospf_cfg->stub_router_startup_time = ospf_mod_row->value_stub_router_adv[i];
            }
    }
...
ovsdb_add_lsa (struct ospf_lsa* lsa)
{
    ...
    ospf_router_row =
        ovsrec_ospf_router_get_by_instance_num(ospf_instance);
    if (!ospf_router_row)
    {
...
//void
//ovsdb_remove_lsa (struct in_addr lsa_id, unsigned char lsa_type, struct ospf_lsa* lsa)
void
ovsdb_remove_lsa (struct ospf_lsa* lsa)
{
    ...
    switch (lsa->data->type)
    {
        case OSPF_ROUTER_LSA:
            ...
            for (i = 0,j = 0; i < area_row->n_router_lsas; i++) {
                if (area_row->router_lsas[i]->ls_id[0] == lsa->data->id.s_addr &&
                    area_row->router_lsas[i]->ls_seq_num[0] == lsa->data->ls_seqnum  &&
                    area_row->router_lsas[i]->adv_router[0] == lsa->data->adv_router.s_addr) {
        ...
        case OSPF_NETWORK_LSA:
            ...
            for (i = 0,j = 0; i < area_row->n_network_lsas; i++) {
                if (area_row->network_lsas[i]->ls_id[0] == lsa->data->id.s_addr &&
                    area_row->network_lsas[i]->ls_seq_num[0] == lsa->data->ls_seqnum  &&
                    area_row->network_lsas[i]->adv_router[0] == lsa->data->adv_router.s_addr) {
                           ...
                           uint32_t full_nbr_count)
{
    ...
    ovs_router = ovsrec_ospf_router_get_by_instance_num(instance);
    if (NULL == ovs_router)
    {
...
{
    struct ovsrec_ospf_interface* ovs_intf = NULL;
    ...
    ovs_intf = find_ospf_interface_by_name(ifname);
    if (NULL == ovs_intf)
    {
        VLOG_DBG ("No associated interface of neighbor");
        ovsdb_idl_txn_abort(nbr_txn);
    ...
    for (i = 0 ; i < ovs_intf->n_neighbors;i++)
    {
        ovs_nbr = ovs_intf->neighbors[i];
        //OPS_TODO : Need to check for network segment if applicable
        //OPS_TODO : Need to check if NSM state is >= Two-way
        if (ovs_nbr)
        {
...
ovsdb_update_nbr (struct ospf_neighbor* nbr)
{
    ...
    ovs_intf = find_ospf_interface_by_name(intf->name);
    if (NULL == ovs_intf)
    {
        VLOG_DBG ("No associated interface of neighbor");
        ovsdb_idl_txn_abort(nbr_txn);
    ...
    ovs_nbr = find_ospf_nbr_by_if_addr(ovs_intf,nbr->src);
    if (!ovs_nbr)
    {
    ...
    //ip_src = nbr->d_router.s_addr;
    //ovsrec_ospf_neighbor_set_dr (ovs_nbr,&ip_src,1);

    //ip_src = nbr->bd_router.s_addr;
    //ovsrec_ospf_neighbor_set_bdr (ovs_nbr,&ip_src,1);
    nbr_id = nbr->router_id.s_addr;
    ovsrec_ospf_neighbor_set_nbr_router_id (ovs_nbr,&nbr_id,1);
    nbr_option = nbr->options;
    ovsrec_ospf_neighbor_set_nbr_options (ovs_nbr,&nbr_option,1);
...
ovsdb_add_nbr (struct ospf_neighbor* nbr)
{
    ...
    new_ovs_nbr = find_ospf_nbr_by_if_addr(ovs_intf,nbr->src);
    if (new_ovs_nbr)
    {
    ...
    ovs_nbr = xmalloc(sizeof * ovs_intf->neighbors *
                                    (ovs_intf->n_neighbors + 1));
    for (i = 0; i < ovs_intf->n_neighbors; i++) {
                 ovs_nbr[i] = ovs_intf->neighbors[i];
    }
    ovs_nbr[ovs_intf->n_neighbors] = new_ovs_nbr;
    ovsrec_ospf_interface_set_neighbors (ovs_intf,ovs_nbr,ovs_intf->n_neighbors + 1);
    nbr_id = nbr->router_id.s_addr;
    ...
    nbr_option = nbr->options;
    ovsrec_ospf_neighbor_set_nbr_options (new_ovs_nbr,&nbr_option,1);
...
ovsdb_delete_nbr (struct ospf_neighbor* nbr)
{
    ...
    old_ovs_nbr = find_ospf_nbr_by_if_addr(ovs_intf,nbr->src);
    if (!old_ovs_nbr)
    {
    ...
    ip_src = nbr->src.s_addr;
    for (i = 0; i < ovs_intf->n_neighbors; i++) {
       if (ip_src != ovs_intf->neighbors[i]->nbr_if_addr)
          ovs_nbr[i] = ovs_intf->neighbors[i];
    }
    ovsrec_ospf_interface_set_neighbors (ovs_intf,ovs_nbr,ovs_intf->n_neighbors - 1);
...
ovsdb_add_area_to_router (int ospf_intance,struct in_addr area_id)
{
    ...
    ospf_router_row =
        ovsrec_ospf_router_get_by_instance_num(ospf_intance);
    if (!ospf_router_row)
    {
       VLOG_DBG ("No OSPF router found");
       ovsdb_idl_txn_abort(area_txn);
                              ...
                              long spf_ts, int spf_count)
{
    ...
    ovs_ospf = ovsrec_ospf_router_get_by_instance_num(instance);
    if (!ovs_ospf)
    {
    ...
    /* Fixme : smap for area_statistics */
...
void ospf_area_tbl_default(const struct ovsrec_ospf_area *area_row)
{
    ...
    char** key_area_type = NULL;
    bool* area_type_value = NULL;
    ...
    /* key_area_type =
        xmalloc(OSPF_STAT_NAME_LEN * (OVS_OSPF_AREA_TYPE_MAX));

    area_type_value =
           xmalloc(sizeof *area_row->value_area_type*
                                 (OVS_OSPF_AREA_TYPE_MAX));*/
    key_area_type =
        xmalloc(OSPF_STAT_NAME_LEN * 1);

    area_type_value =
           xmalloc(sizeof *area_row->value_area_type*
                                 1);

    key_area_type [OVS_OSPF_AREA_TYPE_DEFAULT] = OVSREC_OSPF_AREA_AREA_TYPE_DEFAULT;
    //key_area_type [OVS_OSPF_AREA_TYPE_STUB] = OVSREC_OSPF_AREA_AREA_TYPE_STUB;
    //key_area_type [OVS_OSPF_AREA_TYPE_NSSA] = OVSREC_OSPF_AREA_AREA_TYPE_NSSA;

    area_type_value [OVS_OSPF_AREA_TYPE_DEFAULT] = true;
    //area_type_value [OVS_OSPF_AREA_TYPE_STUB] = false;
    //area_type_value [OVS_OSPF_AREA_TYPE_NSSA] = false;

    //ovsrec_ospf_area_set_area_type(area_row,key_area_type,area_type_value,OVS_OSPF_AREA_TYPE_MAX);
    ovsrec_ospf_area_set_area_type(area_row,key_area_type,area_type_value,1);

    ovsrec_ospf_area_set_nssa_translator_role(area_row, OVSREC_OSPF_AREA_NSSA_TRANSLATOR_ROLE_CANDIDATE);
...
void ospf_interface_if_config_tbl_default (const struct ovsrec_port *ovs_port)
{
                         ...
                         char *ifname)
{
    ...
    ospf_interface_if_config_tbl_default(ovs_port);
...
ospf_is_area_tbl_empty(const struct ovsrec_ospf_area *ospf_area_row)
{
    ...
    {
       /* Since not an smap and internally key name is sorted,
        * can't get by index have to do lookup */
        if (0 == strcmp (ospf_area_row->key_area_type[i],OSPF_AREA_TYPE_NSSA) &&
            true == ospf_area_row->value_area_type[i])
            return false;
        else if (0 == strcmp (ospf_area_row->key_area_type[i],OSPF_AREA_TYPE_STUB) &&
            true == ospf_area_row->value_area_type[i])
            return false;
    }

    if (0 == strcmp (ospf_area_row->nssa_translator_role,OSPF_AREA_NSSA_TRANSLATOR_ROLE_ALWAYS) ||
        0 == strcmp (ospf_area_row->nssa_translator_role,OSPF_AREA_NSSA_TRANSLATOR_ROLE_CANDIDATE))
        return false;
    ...
    if (0 != strcmp(ospf_area_row->ospf_auth_type, OSPF_STRING_NULL))
        return false;
...
void
ospf_area_remove_from_router(int instance,
                              struct in_addr area_id)
{
    ...
    ospf_router_row = ovsrec_ospf_router_get_by_instance_num(instance);
    if (!ospf_router_row)
    {
    ...
    if (ospf_is_area_tbl_empty(ovs_area))
    {
                                      ...
                                      char* ifname)
{
    ...
    for (i = 0, j = 0; i < area_row->n_ospf_interfaces; i++)
    {
        if (strcmp(area_row->key_ospf_interfaces[i],  ifname) != 0)
        {
            ...
            ovs_intf = area_row->value_ospf_interfaces[i];
    }
    if (ovs_intf)
    {
       ovsrec_ospf_area_set_ospf_interfaces(area_row, interface_list, ospf_interface_list,
                                               (area_row->n_ospf_interfaces - 1));
       ovs_port = ovs_intf->port;

       ovsrec_ospf_interface_delete(ovs_intf);
...
#if 0
        ospf_area_apply_changes (idl) |
        ospf_interface_apply_changes(idl) |
#endif
