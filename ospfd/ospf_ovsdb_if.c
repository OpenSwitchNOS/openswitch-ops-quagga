    ...
    const struct ovsrec_ospf_router *ospf_mod_row)
{
    ...
    admin_set = smap_get_bool(&(ospf_mod_row->stub_router_adv),
                              OVSREC_OSPF_ROUTER_STUB_ROUTER_ADV_ADMIN,false);
...
void
ovsdb_update_nbr_dr_bdr (struct in_addr if_addr,
...
void
ovsdb_add_nbr_self (struct ospf_neighbor* nbr, char* intf)
{
    struct ovsrec_ospf_interface* ovs_intf = NULL;
    struct ovsrec_ospf_neighbor** ovs_nbr = NULL;
    struct ovsrec_ospf_neighbor* new_ovs_nbr = NULL;
    ...
    ovs_intf = find_ospf_interface_by_name(intf);
    if (NULL == ovs_intf)
    {
        VLOG_DBG ("No associated interface of neighbor");
        ovsdb_idl_txn_abort(nbr_txn);
...
void
ovsdb_set_nbr_self_router_id (char* ifname, struct in_addr if_addr,
                                                struct in_addr router_id)
{
    ...
    ovs_nbr = find_ospf_nbr_by_if_addr(ovs_intf,if_addr);
    if (!ovs_nbr)
    {
...
void
ovsdb_delete_nbr_self (struct ospf_neighbor* nbr, char* ifname)
{
     struct ovsrec_ospf_interface* ovs_intf = NULL;
    struct ovsrec_ospf_neighbor** ovs_nbr = NULL;
    struct ovsrec_ospf_neighbor* old_ovs_nbr = NULL;
                                      ...
                                      char* ifname)
{
    ...
    for (i = 0, j = 0; i < area_row->n_ospf_interfaces; i++)
    {
        if (strcmp(area_row->ospf_interfaces[i]->name,  ifname) != 0)
        {
            ...
            ovs_intf = area_row->ospf_interfaces[i];
    }
    if (ovs_intf)
    {
       ovsrec_ospf_area_set_ospf_interfaces(area_row, ospf_interface_list,
