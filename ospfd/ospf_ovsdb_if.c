...
int
ospf_lsa_timer_refresh_interval_set(struct ospf *ospf_cfg, int interval)
{
    int time_left;
    ...
    const struct ovsrec_ospf_router *ospf_mod_row)
{
   ...
   metric = smap_get_int(&(ospf_mod_row->other_config), "default_metric",
             ...
             const struct ovsrec_ospf_router *ospf_mod_row)
{
    ...
    if (timers == OSPF_LSA_REFRESH_INTERVAL_DEFAULT)
        ospf_lsa_timer_refresh_interval_set(ospf_cfg,
                                             OSPF_LSA_REFRESH_INTERVAL_DEFAULT);
    else
        ospf_lsa_timer_refresh_interval_set(ospf_cfg, timers);
                                                   ...
                                                   struct ospf_area* area)
{
    ...
    if (cost == atoi(OSPF_AREA_STUB_DEFAULT_COST))
        area->default_cost = atoi(OSPF_AREA_STUB_DEFAULT_COST);
    else
