                   ...
                   const struct ovsrec_route *ovs_rib, struct ospf_route *or)
{
    ...
    //Add OVSREC_* enum string if more enum added
    ...
    n_nexthop = listcount(or->paths);
    if (!n_nexthop)
    {
        VLOG_DBG ("No nexthop present for the route");
        return -1;
    }
    nexthop_list = xmalloc(sizeof *ovs_rib->nexthops * n_nexthop);
    if (!nexthop_list)
    {
    ...
    ovsrec_route_set_nexthops(ovs_rib, nexthop_list, n_nexthop);
    free(nexthop_list);
    return 0;
}
...
ovsdb_ospf_add_rib_entry (struct prefix_ipv4 *p, struct ospf_route *or)
{
    ...
    char* ad_family = OVSREC_ROUTE_ADDRESS_FAMILY_IPV4;
    ...
    ovsrec_route_set_address_family(ovs_rib,ad_family);
    ...
    ovsrec_route_set_prefix(ovs_rib,pr);
    ...
    if (TXN_SUCCESS != status &&
        ...
        if (rt_uuid)
            shash_add(&all_routes,pr,(void *)rt_uuid);
    }
    ovsdb_idl_txn_destroy(txn);
}
...
void
modify_ospf_interface_instance (struct ovsdb_idl *idl,
    const struct ovsrec_port* ovs_port, const char* if_name)
{
    const struct ovsdb_idl_column *column = NULL;
    ...
    int i = 0;
     ...
     OVSREC_OSPF_INTERFACE_FOR_EACH(ovs_oi, idl) {
        ...
        if (ovs_port && OVSREC_IDL_IS_ROW_MODIFIED(ovs_port, idl_seqno))
           modify_ospf_interface_instance(idl, ovs_port,ovs_oi->name);
     }
}
