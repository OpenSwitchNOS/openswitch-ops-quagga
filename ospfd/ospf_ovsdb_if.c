                     ...
                     struct external_info *ei)
{
    ...
    if (ei) {
       ...
       VLOG_DBG ("No external info present");
       return 1;
    }
    return 0;
}
    ...
    const struct ovsrec_ospf_area* ovs_area, int64_t area_id, int64_t ospf_inst)
{
    ...
    if (OVSREC_IDL_IS_COLUMN_MODIFIED(ovsrec_ospf_area_col_area_type, idl_seqno)) {
        if (!ovs_area->area_type ||
            ...
            if (!OSPF_IS_AREA_ID_BACKBONE(areaid)) {
                ...
                if (ret != 0)
                    VLOG_DBG ("Error setting area as stub. Area has virtual links through it");
                else {
                ...
                VLOG_DBG ("Cannot configure Backbone area as stub");
        }
        else if(!strcmp(ovs_area->area_type,OVSREC_OSPF_AREA_AREA_TYPE_STUB_NO_SUMMARY)) {
            if (!OSPF_IS_AREA_ID_BACKBONE(areaid)) {
                ret = ospf_area_vlink_count (ospf, area);
                if (ret != 0)
                    VLOG_DBG ("Error setting area %d as stub. Area has virtual links through it");
                else {
            ...
            if (!OSPF_IS_AREA_ID_BACKBONE(areaid)) {
                ret = ospf_area_vlink_count (ospf, area);
                if (ret != 0)
                    VLOG_DBG ("Error setting area %d as NSSA. Area has virtual links through it");
                else {
                ...
                VLOG_DBG ("Cannot configure Backbone area as NSSA");
        }
    }
    if (OVSREC_IDL_IS_COLUMN_MODIFIED(ovsrec_ospf_area_col_nssa_translator_role, idl_seqno)) {
        if (area->external_routing == OSPF_AREA_NSSA) {
            ...
            VLOG_DBG ("Area is not configured as NSSA");
    }
    ...
    if (OVSREC_IDL_IS_COLUMN_MODIFIED(ovsrec_port_col_ospf_auth_md5_keys, idl_seqno)) {
        for (i = 0 ; i < ovs_port->n_ospf_auth_md5_keys;i++) {
            ...
            if (ospf_crypt_key_lookup (params->auth_crypt, key_id) != NULL) {
                VLOG_DBG ("OSPF MD5 key already present");
                continue;
            }
            else {
...
ospf_apply_route_changes (struct ovsdb_idl *idl)
{
    ...
    */
    ...
    if (!ospf_instance)
    {
        VLOG_DBG ("No OSPF instance found to apply route changes");
        return 0;
    }
...
ospf_apply_nh_changes (struct ovsdb_idl *idl)
{
     ...
     */
     ...
     if (!ospf_instance)
     {
         VLOG_DBG ("No OSPF instance found to apply NH changes");
         return 0;
     }
