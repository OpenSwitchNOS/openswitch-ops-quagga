        ...
        if (path->ifindex != 0)
        {
            ...
            if (ifname)
            {
                ...
                if (ovs_port)
                {
                    if (ovs_port->name)
                       snprintf(temp_str + len_str,99 - len_str,
                                " port:%s",ovs_port->name);
                    ovsrec_nexthop_set_ports(pnexthop,&ovs_port,1);
                }
            }
        }
        strcat (temp_str,",");
        pnexthopstr = realloc(pnexthopstr,
                              sizeof(pnexthopstr) + strlen(temp_str) +1);
...
ovsdb_ospf_add_rib_entry (struct prefix_ipv4 *p, struct ospf_route *or)
{
    ...
    /* Event logging */
    snprintf (temp_str,99," distance:%d metric:%d", distance, metric);
    por_str = realloc(next_hop_str,sizeof(next_hop_str)+strlen(temp_str)+1);
    strncat (por_str, temp_str, strlen(temp_str));
    log_event("OSPFv2_ROUTE",
              EV_KV("event","OSPFv2 ROUTE ADD"),
              EV_KV("prefix","%s",prefix_str),
              EV_KV("route","%s",por_str));
    if (por_str)
      free(por_str);
