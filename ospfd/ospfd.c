...
#ifdef ENABLE_OVSDB
        ovsdb_set_nbr_self_router_id(oi->ifp->name, oi->nbr_self->src,
...
#ifdef ENABLE_OVSDB
ospf_area_remove_from_router(area->ospf->ospf_inst,
                              area->area_id);
#endif
  /* Free LSDBs. */
...
#ifdef ENABLE_OVSDB
      ovsdb_add_area_to_router (ospf->ospf_inst, area_id);
#endif
    }
      ...
      && ! ospf_if_table_lookup(ifp, co->address)
          && ospf_network_match_iface(co,p))
        {
           struct ospf_interface *oi;
            ...
            ovsdb_add_nbr_self(oi->nbr_self, oi->ifp->name);
#endif
...
ospf_network_run (struct prefix *p, struct ospf_area *area)
{
  ...
  for (ALL_LIST_ELEMENTS_RO (om->iflist, node, ifp))
  /* Do not call this if interface is part of lag */
    ospf_network_run_interface (p, area, ifp);
}

#ifdef OPS_QUAGGA
static void
ospf_run_interface (struct ospf_area *area, struct interface *ifp)
{
  struct listnode *cnode;
  struct connected *co;
...
int
ospf_interface_area_set (struct ospf *ospf, struct ospf_area *area, const char *ifname)
{
  struct interface *ifp;

  assert (ospf && area && ospf == area->ospf);

  if (!(ifp = if_lookup_by_name (ifname))) {
    zlog_err ("No interface - %s", ifname);
    return -1;
  }

  ospf_run_interface (area, ifp);

  return 0;
}

int
ospf_interface_area_unset (struct ospf *ospf, struct ospf_area *area, const char *ifname)
{
  struct interface *ifp;
  struct ospf_interface *oi;
  struct listnode *cnode;
  ...
  for (ALL_LIST_ELEMENTS_RO (ifp->connected, cnode, co)) {
    ...
    if ((AF_INET == co->address->family) && ((oi = ospf_if_table_lookup(ifp, co->address)) != NULL)) {
      ospf_if_free (oi);
