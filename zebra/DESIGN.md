...
* Zebra takes all the advertised routes into account and internally decides best routes.
* On selecting/unselecting an active route, Zebra creates FIB and update the kernel with these routes.
* This selected routes are also communicated to ops-vswitchd through OVSDB for further programming them in ASIC.
* And to handle routes and nexthop deletes, on getting delete notification from OVSDB,  Zebra creates a local hash of current routes and nexthop, and compares them with its local RIB/FIB storage, and removes the deleted ones from its storage and kernel.
