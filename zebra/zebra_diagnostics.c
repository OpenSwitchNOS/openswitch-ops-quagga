...
static int
parse_dump_options(int argc, const char *argv[], char *return_status,
                   struct unixctl_conn *conn)
{
    if (argc > 2) {
        sprintf(return_status, "Wrong argument count: %d", argc);
...
zebrad_internals_dump(char *buf, int buf_size, const char *dump_option)
{
    ...
    if (!dump_option || !strcmp(dump_option, "rib")) {
        zebra_dump_formatted_string(&ds, "\n-------- Zebra internal IPv4 routes dump: --------\n");
        zebra_dump_internal_route_table(&ds, vrf_table(AFI_IP,
                                        SAFI_UNICAST, 0));
        zebra_dump_formatted_string(&ds, "\n-------- Zebra internal IPv6 routes dump: --------\n");
        ...
        const char *argv[] OVS_UNUSED, void *aux OVS_UNUSED)
{
    ...
    if (parse_dump_options(argc, argv, err_str, conn)) {
        unixctl_command_reply_error(conn, err_str);
        return;
    }
    char *buf = xcalloc(1, zebra_diag_buffer_len);
    if(!buf) {
        snprintf(err_str,sizeof(err_str),
                 "zebra daemon failed to allocate %d bytes",
                 zebra_diag_buffer_len);
        unixctl_command_reply(conn, err_str);
    } else {
        zebrad_internals_dump(buf, zebra_diag_buffer_len, argv[1]);
...
{
    /* Initialize basic diagnostics framework for zebra */
    INIT_DIAG_DUMP_BASIC(zebra_diag_dump_basic_cb);

    /* Register ovs-appctl commands for this daemon. */
    unixctl_command_register("zebra/dump", "rib|kernel-routes|l3-port-cache",
                             0, 1, zebra_unixctl_dump, NULL);
 ...
 */
void
zebra_dump_internal_nexthop (struct ds *ds, struct prefix *p, struct nexthop* nexthop)
{
...
zebra_dump_internal_rib_entry (struct ds *ds, struct prefix *p, struct rib* rib)
{
    ...
    zebra_dump_formatted_string(ds, "\tRoute type: %s Metric: %u Distance: %u Number: %u\n",
...
zebra_dump_internal_route_node (struct ds *ds, struct route_node *rn)
{
    ...
    if (!rn) {
        zebra_dump_formatted_string(ds, "Route node is null\n");
        return;
    }
    ...
    if (!p) {
        zebra_dump_formatted_string(ds, "Prefix is null\n");
        return;
    }
...
zebra_dump_internal_route_table (struct ds *ds, struct route_table *table)
{
    ...
    if (!table) {
        zebra_dump_formatted_string(ds, "The table is null");
        return;
    }
...
zebra_l3_port_node_print (struct ds *ds, struct zebra_l3_port* l3_port)
{
    ...
    zebra_dump_formatted_string(ds, "Printing the L3 node for Port name: %s\n",
             l3_port->port_name ? l3_port->port_name : "NULL");
    zebra_dump_formatted_string(ds, "\tPort Primary IP Address: %s\n",
                                l3_port->ip4_address ? l3_port->ip4_address :
                                "NULL");
    zebra_dump_formatted_string(ds, "\tPort Primary IPv6 Address: %s\n",
                                l3_port->ip6_address ? l3_port->ip6_address :
                                "NULL");

    zebra_dump_formatted_string(ds, "\tOVSDB port UUID is: %u-%u-%u-%u\n",
                                    ...
                                    bool if_permanent_hash)
{
    ...
    zebra_dump_formatted_string(ds, "Walking the L3 port cache to print all L3 ports in %s cache\n",
            if_permanent_hash ? "permanent" : "temporary");

    if (!shash_count(zebra_cached_l3_ports)) {
        zebra_dump_formatted_string(ds, "The hash table is empty. Nothing to walk and print\n");
        return;
    }
