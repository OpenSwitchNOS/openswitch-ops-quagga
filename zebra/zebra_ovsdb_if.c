...
static char* get_my_netns() {
  ...
  fp = popen(command,"r");
  fscanf(fp,"%s",line);
  if (strncmp(line,vrf_s,strlen(vrf_s)) == 0) {
      memset(line, 0, 32);
      memcpy(line,vrf_d,strlen(vrf_d));
  }
  return line;
}
...
zebra_if_ovsrec_port_is_l3 (const struct ovsrec_port* port)
{
  ...
  OVSREC_VRF_FOR_EACH (vrf_row, idl)
    {
      #ifdef VRF_ENABLE
      if (strncmp(vrf_row->name,my_netns,strlen(vrf_row->name)))
        continue;
      #endif
...
ovsdb_init (const char *db_path)
{
  ...
  if (strcmp(my_netns, "vrf_default"))
    {
...
#ifdef VRF_ENABLE
boolean is_route_inmy_vrf (const struct ovsrec_route *ovs_route)
{
  char *vrf_name = NULL;
  ...
  return(!strncmp(ovs_rt_vrf,my_netns,strlen(ovs_rt_vrf)));
}
#endif
   ...
   switch (PREFIX_FAMILY (p))
     {
       case AF_INET:
         ...
         OVSREC_ROUTE_FOR_EACH (ovs_route, idl)
	   {
             if (!(is_route_inmy_vrf(ovs_route)))
               continue;
