      ...
      if (!shash_add(&zebra_cached_l3_ports, ovsrec_port->name,
                     l3_port))
        {
          ...
          log_event("ZEBRA_PORT", EV_KV("port_msg", "%s",
                    "Added L3 port to the hash successfully "),
      ...
      if (!zebra_if_ovsrec_port_is_l3(ovsrec_port) &&
                              zebra_if_port_is_l3(l3_port))
        {
          VLOG_DBG("Port changed from L3 to L2. Deleting the port %s"
                   " from the hash\n", ovsrec_port->name);
          log_event("ZEBRA_PORT", EV_KV("port_msg", "%s",
                    "Port changed from L3 to L2. Deleting the port "),
                    EV_KV("port_name", "%s", ovsrec_port->name));
                                                 ...
                                                 (ovsrec_port, l3_port))
        {
          ...
          log_event("ZEBRA_PORT", EV_KV("port_msg", "%s",
                    "Port IPv4 changed. Update the port IP address in "
                    "the hash and add a node in the hash"),
                    EV_KV("port_name", "%s", ovsrec_port->name));
          /*
      ...
      else
        {
          ...
          log_event("ZEBRA_PORT", EV_KV("port_msg", "%s",
                    "No change in configuration for this port"),
                    EV_KV("port_name", "%s", ovsrec_port->name));
      ...
      if ((l3_port->port_action ==
          ...
          (l3_port->port_action == ZEBRA_L3_PORT_DELETE))
        {
          ...
          log_event("ZEBRA_NEXTHOP_STATE", EV_KV("nexthop_port", "%s",
                    nexthop_str), EV_KV("msg", "%s", "now deleted"));
          return(true);
        }
    }
                        ...
                        if (!zebra_nh_port_active_in_cached_l3_ports_hash(ifname))
                          {
                            ...
                            log_event("ZEBRA_NEXTHOP_STATE",
                                      ...
                                      EV_KV("msg", "%s","marked as unselected"));
                        ...
                        else
                          {
                            ...
                            log_event("ZEBRA_NEXTHOP_STATE",
                                      EV_KV("nexthop_port", "%s",ifname),
                                      EV_KV("msg", "%s","marked as selected"));
                            zebra_update_selected_nh(rn, rib, ifname, NULL,
                                                     ZEBRA_NH_INSTALL);
                          }
                        break;
                      ...
                      case ZEBRA_L3_PORT_DELETE_UPDATE_OPTION:
                        ...
                        if (zebra_nh_address_in_deleted_cached_l3_ports_hash(
                                                         nexthop_str, afi))
                          {
                            ...
                            log_event("ZEBRA_NEXTHOP_STATE", EV_KV("nexthop_port",
                                      "%s",nexthop_str), EV_KV("msg", "%s",
                                      "found in deleted l3 port list"));
                            zebra_route_list_add_data(rn, rib, nexthop);
          ...
          if ( (OVSREC_IDL_IS_ROW_INSERTED(route_row, idl_seqno)) ||
               ...
               (is_route_nh_rows_modified(route_row)) )
            {
              ...
              log_event("ZEBRA_ROUTE", EV_KV("routemsg", "%s",
                        "Row modification or inserts in ROUTE table"),
                        EV_KV("prefix", "%s", route_row->prefix));
          ...
          if (strcmp(nexthop_str, nh_row->ip_address) == 0)
            {
              ...
              log_event("ZEBRA_NEXTHOP_STATE", EV_KV("nexthop_port", "%s",
                        nexthop_str), EV_KV("msg", "%s", "going to be deleted"));
            }
        }
          ...
          if (!(nh_row->ip_address) && !(nh_row->n_ports))
                       ...
                       && nh_row->ports[0]->name)
            {
              ...
              if (strcmp(nh_row->ports[0]->name, port_name) == 0) {
                  found_port[next_hop_index] = true;
                  log_event("ZEBRA_NEXTHOP_STATE", EV_KV("nexthop_port", "%s",
                            port_name), EV_KV("msg", "%s", "going to be deleted"));
              }
            }
        }
  ...
  if (ovs_route)
    {
      ...
      log_event("ZEBRA_ROUTE", EV_KV("routemsg", "%s",
                "Updating selected route for"), EV_KV("prefix", "%s",
                ovs_route->prefix));
      /*
