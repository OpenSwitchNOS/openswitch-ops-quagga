...
/*
 * This function returns ipv4/ipv6 prefix and prefix length. In case of
 * successful conversion, this function return 0 otherwise it returns -1.
 */
static int
zebra_get_prefix(int family, char *ip_address, void *prefix,
                 unsigned char *prefixlen)
{
    char *p;
    char *ip_address_copy;
    int maxlen = (family == AF_INET) ? ZEBRA_MAX_IPV4_MASKLEN :
                                       ZEBRA_MAX_IPV6_MASKLEN;
    *prefixlen = maxlen;
  ...
  if (is_ipv4)
    {
       ...
       */
      retval = zebra_get_prefix(AF_INET, (char*)ip_address,
  ...
  else
    {
       ...
       * Split address/subnet string into address and string
       */
      retval = zebra_get_prefix(AF_INET6, (char*)ip_address,
  ...
  OVSREC_VRF_FOR_EACH (row_vrf, idl)
    {
      ...
      for (iter = 0; iter < count; iter++)
        {
          if (0 == strncmp(port_name, row_vrf->ports[iter]->name, 32))
            return row_vrf;
        }
  ...
  if (is_v4)
    {
      ...
      prefix_str = zebra_convert_ip_addr_to_prefic_format(ip_address, true);
      ...
      if (prefix_str)
        ...
        {
          VLOG_DBG("Unable to convert IPv4 address %s into prefix format",
                   ip_address);

          /*
           * TODO: Check if we need to release memory for route row.
           */
          return;
        }
    }
  else
    {
      ...
      prefix_str = zebra_convert_ip_addr_to_prefic_format(ip_address, false);
      ...
      if (prefix_str)
        ...
        {
          VLOG_DBG("Unable to convert IPv6 address %s into prefix format",
                   ip_address);
                                                 ...
                                                 struct zebra_l3_port** l3_port)
{
  ...
  /*
   * Add/delete IPv4 and IPv6 primary addresses and their corresponding
   * static routes.
   */
  ...
  /*
   * Add/delete IPv4 and IPv6 secondary addresses and their corresponding
   * static routes.
   */
...
zebra_delete_connected_routes_from_deleted_l3_port_cache ()
{
  ...
  if (!shash_count(&zebra_updated_or_changed_l3_ports))
    {
      VLOG_DBG("The port delete hash table is empty. No connected routes to delete");
      return;
    }

  /*
   * Allocate empty OVSDB port node
   */
