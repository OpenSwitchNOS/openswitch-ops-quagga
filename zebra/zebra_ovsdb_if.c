...
static int zebra_update_neighbor(const char *route_prefix,
                                 const char *nh_ip_address,
                                 void *data, bool is_selected);
#ifdef HAVE_IPV6
extern int
  ...
  for (ALL_LIST_ELEMENTS (zebra_route_del_list, node, nnode, rdata))
    {
	  ...
	  /* Ignore broadcast and multicast routes */
          ...
          if (pprefix->family == AF_INET)
	    {
              ...
              VLOG_DBG("Check for nexthop type%d",rdata->nexthop->type);
...
zebra_syncup_neighbors(void)
{
  ...
  /* Look for any modification of mac/port of neighbors */
...
static int zebra_update_neighbor_route_ref(struct neighbor *neighbor,
                        ...
                        const char *route_prefix, void *route_data,
                        bool add_ref)
{
  bool in_use = false;

  VLOG_DBG("In %s for neighbor %s and route %s and ref_flag %s",
           __func__, neighbor->ip_address, route_prefix,
           add_ref ? "ADD" : "REMOVE");
  if (add_ref)
    {
                                 ...
                                 bool is_selected)
{
  ...
  if (neighbor)
  ...
  else
    {
      ...
      ** resolution */
      route_row = (const struct ovsrec_route *)route_data;
      if (is_selected)
        {
          ...
          if (new_neighbor)
            {
              ...
              /* Add the route ref to the neighbor */
