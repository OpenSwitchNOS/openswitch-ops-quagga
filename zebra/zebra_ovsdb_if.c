...
{
  ZEBRA_L3_PORT_DELETE_OPTION,
  ZEBRA_L3_PORT_ADMIN_CHN_OPTION
};
 ...
 */
static bool
zebra_if_ovsrec_port_is_l3 (struct ovsrec_port* port)
{
  if (!port)
    return(false);
...
zebra_get_port_active_state (
                    ...
                    const struct ovsrec_interface* candidate_interface)
{
  ...
  if (!port)
    {
      VLOG_DBG("The port pointer is NULL");
      return(active_state);
    }

  if (!(port->name))
    {
      VLOG_DBG("The port name pointer is NULL");
      return(active_state);
    }

  if (candidate_interface)
    {
      if (candidate_interface->name &&
          (strcmp(candidate_interface->name, port->name) == 0))
        {
          if (candidate_interface->admin_state &&
              (strcmp(candidate_interface->admin_state,
                        OVSREC_INTERFACE_ADMIN_STATE_UP) == 0))
            {
              VLOG_DBG("Found an interface %s in admin up state. "
                        "Setting the port %s in active state",
                        candidate_interface->name, port->name);
              active_state = true;
            }
          else
            {
              VLOG_DBG("Found an interface %s in admin down state. "
                        "Setting the port %s in inactive state",
                        candidate_interface->name, port->name);
            }
        }
      else
        {
          VLOG_DBG("THe candidate interface name %s and the "
                    "port name %s do match",
                    candidate_interface->name, port->name);
        }
    }
  else
    {
      OVSREC_INTERFACE_FOR_EACH (interface, idl)
        {
          if (!(interface->name))
            {
          ...
          VLOG_DBG("Testing interface %s to match with "
                    "port %s\n", interface->name, port->name);

          if (strcmp(interface->name, port->name) == 0)
            {
              VLOG_DBG("     Found a match for %s in the "
                        "interface table", port->name);
              active_state = true;
              break;
            }
        }
...
zebra_l3_port_node_create (struct ovsrec_port* ovsrec_port)
{
  ...
  memset(l3_port, 0, sizeof(struct zebra_l3_port));
  ...
  for (secondary_addr_index = 0;
       ...
       ++ secondary_addr_index)
    {
      ...
      if (!shash_add(
        ...
        {
          VLOG_DBG("Unable to add the secondary IP address %s to "
                    "L3 port hash table",
                    ovsrec_port->ip4_address_secondary[secondary_addr_index]);
          continue;
        }
  ...
  for (secondary_addr_index = 0;
       ...
       ++ secondary_addr_index)
    {
      ...
      if (!shash_add(
        ...
        {
          VLOG_DBG("Unable to add the secondary IPv6 address %s to "
                    "L3 port hash table",
                    ovsrec_port->ip6_address_secondary[secondary_addr_index]);
          continue;
        }
...
zebra_l3_port_node_free (struct zebra_l3_port* l3_port)
{
  ...
  if (!l3_port)
    {
      VLOG_DBG("The L3 port structure is NULL. Nothing to free");
      return;
    }
 ...
 */
static bool
zebra_get_if_port_updated_or_changed ()
{
 ...
 */
static void
zebra_add_or_update_cached_l3_ports_hash (struct ovsrec_port* ovsrec_port)
{
  /*
  ...
  if (!l3_port)
    {
      ...
      if (!l3_port)
        {
          VLOG_DBG("Unable to add L3 port to the hash");
          return;
        }
      ...
      if (!zebra_if_ovsrec_port_is_l3(ovsrec_port) &&
                              zebra_if_port_is_l3(l3_port))
        {
          ...
          if (!shash_add(&zebra_updated_or_changed_l3_ports,
                         ...
                         l3_port))
            {
              VLOG_DBG("Unable to add L3 port to the temp hash");
              zebra_l3_port_node_free(l3_port);
            }
          else
            {
  ...
  SHASH_FOR_EACH_SAFE (node, next, &zebra_cached_l3_ports)
    {
      ...
      if (!(node->data))
        {
          VLOG_DBG("No data found in the node\n");
          continue;
        }
                                         ...
                                         char* port_name)
{
  ...
  if (!port_name)
    {
      VLOG_DBG("The port name is NULL");
      return;
    }
                                       ...
                                       char* nexthop_str, afi_t afi)
{
  ...
  if (!nexthop_str || !nexthop_str[0])
    {
      VLOG_DBG("The nexthop string is null");
      return(NULL);
    }
      ...
      SHASH_FOR_EACH_SAFE (node, next, port_hash)
        {
          if (!node)
            {
              VLOG_DBG("No node found in the L3 port "
                        "hash\n");
              continue;
            }
          ...
          if ((afi == AFI_IP) && !((l3_port->ip4_address) ||
            ...
            {
              VLOG_DBG("Mismatch between address family "
                        "and IPv4 address");
              continue;
            }
                ...
                (shash_count(&(l3_port->ip4_address_secondary)))))
            {
              ...
              SHASH_FOR_EACH_SAFE (
                  ...
                  &(l3_port->ip4_address_secondary))
                {
                  if (!secondary_node)
                    {
                      VLOG_DBG("No node found in the L3 "
                                "port hash\n");
                      continue;
                    }
...
zebra_l3_port_node_lookup_by_interface_name (char* interface_name)
{
  ...
  l3_port = (struct zebra_l3_port *)shash_find_data(
                                          &zebra_cached_l3_ports,
                                          interface_name);
...
static void
zebra_update_port_active_state (struct ovsrec_interface *interface)
{
  struct zebra_l3_port* l3_port = NULL;
                        ...
                        enum zebra_handle_port_op_options option)
{
  ...
  if (!zebra_get_if_port_updated_or_changed()
      && !zebra_get_if_port_active_state_changed())
    {
      VLOG_DBG("The deleted ports hash table is empty. No nexthops to delete");
      return;
    }
          ...
          for (nexthop = rib->nexthop; nexthop; nexthop = nexthop->next)
            {
              ...
              if (ifname[0])
                {
                  if (option == ZEBRA_L3_PORT_DELETE_OPTION)
                    {
                  ...
                  if (option == ZEBRA_L3_PORT_ADMIN_CHN_OPTION)
                    {
                      if (!zebra_nh_port_active_in_cached_l3_ports_hash(ifname))
                        {
                          ...
                          zebra_update_selected_nh_port_or_addr(rn, rib, ifname,
                                                                NULL,
                                                                ZEBRA_NH_UNINSTALL);
                        }
                      else
                  ...
                  if (option == ZEBRA_L3_PORT_ADMIN_CHN_OPTION)
                    {
                      if (!zebra_nh_addr_active_in_cached_l3_ports_hash(
                                                              nexthop_str, afi))
                        {
                          ...
                          zebra_update_selected_nh_port_or_addr(rn, rib, NULL,
                                                                nexthop_str,
                                                                ZEBRA_NH_UNINSTALL);;
                        }
                      else
...
zebra_handle_route_change (const struct ovsrec_route *route)
{
  ...
  switch (from_protocol)
    ...
    case ZEBRA_ROUTE_CONNECT:
      ...
      /* OPS_TODO: Might have to delete this case */
      break;
 ...
 * eventually marks the next-hops as selected or unselected.
 */
static void
zebra_handle_interface_admin_state_changes ()
{
  VLOG_DBG("Inside function %s at line %d\n",
  ...
  /*
   * If some L3 port became active or iactive due to intrface unshut pr
   * shut, then walk all zebra routes and mark the appropriate next-hops
   * as selected or unselected in the OVSDB.
   */
  ...
  if (!first_port_row)
    {
      ...
      /*
       * OPS_TODO: Cleanup all static routes created by zebra in kernel
       */
      return;
    }
...
/*
 * his is a helper function which deletes the OVSDB next-hop entry from the
 * route. If the number of next-hops for the route becomes 0, then the route
                             ...
                             size_t number_next_hops)
{
  ...
  if (!route || !nexthop_decision)
    {
      VLOG_DBG("Either the route or the nexthop is NULL");
      return;
    }

  if (route->n_nexthops != number_next_hops)
    {
        VLOG_DBG("The number of route nexthops %u is not same as decision"
                  " next-hops %u", route->n_nexthops, number_next_hops);
        return;
    }
...
/*
 * This function takes a zebra rib entry and the next-hop
 * port and deletes the corresponding next-hop port from OVSDB.
 */
                                              ...
                                              char* nexthop_str)
{
  ...
  if (!route || !nexthop_str)
    {
      VLOG_DBG("The route's rib entry or next-hop address in null");
      return;
    }
  ...
  if (route && (route->ovsdb_route_row_ptr))
    {
      ...
      VLOG_DBG("Walking the route next-hops to find the next-hop pointer"
               " corresponding to the next-hop address %s", nexthop_str);
      ...
      for (next_hop_index = 0; next_hop_index < route_row->n_nexthops;
           ++next_hop_index)
        {
          ...
          if (!(nh_row->ip_address))
            {
              VLOG_DBG("NULL next-hop ip address");
              continue;
            }
...
/*
 * This function takes a zebra route node, rib entry and the next-hop
 * port and deletes the corresponding next-hop port from OVSDB.
 */
                                              ...
                                              char* port_name)
{
  ...
  if (!route || !port_name)
    {
      VLOG_DBG("The route's rib entry or port name in null");
      return;
    }
...
/*
 * This function takes a zebra rn, zebra rib entry and the next-hop
 * port or IP/IPv6 address and the selected bit. The function then
 * looks for the corresponding next-hop row in the OVSB and marks the
 * next-hop as true or false depending on the selected value.
 */
                                            ...
                                            int selected)
{
  ...
  int next_hop_index;
  bool if_selected = (ZEBRA_NH_INSTALL == selected) ? true : false;
  int number_of_selected_nh = 0;

  /*
   * If the rib entry in NULL and both of port_name and nh_addr
   * are NULL, then return from the function.
   */
  if (!rn || !route || (!port_name && !nh_addr))
    {
      VLOG_DBG("The route node or route's rib entry in null");
      return;
    }
  ...
  if (route->ovsdb_route_row_ptr)
    {
      ...
      for (next_hop_index = 0; next_hop_index < route_row->n_nexthops;
           ++next_hop_index)
        {
          ...
          if (!(nh_row->selected) || (nh_row->selected[0]))
            {
      ...
      if (cand_nh_row)
        {
          if (!(cand_nh_row->selected))
            {
              VLOG_DBG("Changing the next-hop selected flag from %s to %s",
                        ...
                        if_selected ? "true" : "false");
              ovsrec_nexthop_set_selected(cand_nh_row, &if_selected, 1);
              zebra_txn_updates = true;
            }
          else
            {
              if (cand_nh_row->selected[0] != if_selected)
                {
                  VLOG_DBG("Changing the next-hop selected flag from %s to %s",
          ...
          VLOG_DBG("The number of actie next-hops are %d",
                    number_of_selected_nh);
              ...
              if (number_of_selected_nh)
                {
                  ...
                  zebra_update_selected_route_to_db(rn, route, ZEBRA_RT_INSTALL);
                }
            }
          else
            {
              ...
              /*
               * If no next-hop is selected, then mark the
               * route as unselected.
               */
              if (!number_of_selected_nh)
                {
                  ...
                  zebra_update_selected_route_to_db(rn, route, ZEBRA_RT_UNINSTALL);
                }
            }
        }
    }
