 ...
 */
static bool
get_vrf_from_uuid (char *uuid, char *vrf)
{
 ...
 */
static bool
is_route_in_my_vrf (const struct ovsrec_route *ovs_route)
{
  char *vrf_name = NULL;
  ...
  if (!zebra_vrf || !strlen(zebra_vrf))
    {
      ...
      VLOG_DBG("zebra_ovs_run for vrf %s\n", zebra_vrf);
      if (!zebra_vrf)
        {
  ...
  if (route && (route->ovsdb_route_row_ptr))
    {
      ...
      #ifdef VRF_ENABLE
      if (!(is_route_in_my_vrf(route_row)))
        return;
      #endif
