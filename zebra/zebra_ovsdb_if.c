          ...
          if (pprefix->family == AF_INET)
	    {
              if (rdata->rib->type == ZEBRA_ROUTE_STATIC)
                ...
                if ((NEXTHOP_TYPE_IPV4_IFINDEX == rdata->nexthop->type) ||
                    (NEXTHOP_TYPE_IFINDEX == rdata->nexthop->type))
                    nh_ifindex = rdata->nexthop->ifindex;
                else if ((NEXTHOP_TYPE_IPV4_IFNAME == rdata->nexthop->type) ||
                    (NEXTHOP_TYPE_IFNAME == rdata->nexthop->type))
	  ...
	  else if (pprefix->family == AF_INET6)
	    {
              if (rdata->rib->type == ZEBRA_ROUTE_STATIC)
                ...
                if ((NEXTHOP_TYPE_IPV6_IFINDEX == rdata->nexthop->type) ||
                    (NEXTHOP_TYPE_IFINDEX == rdata->nexthop->type))
                    nh_ifindex = rdata->nexthop->ifindex;
                else if ((NEXTHOP_TYPE_IPV6_IFNAME == rdata->nexthop->type) ||
                    (NEXTHOP_TYPE_IFNAME == rdata->nexthop->type))
                    nh_ifindex = ifname2ifindex(rdata->nexthop->ifname);
